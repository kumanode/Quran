...
import com.quran.app.compose.theme.colors.BaseColors
import com.quran.app.compose.theme.colors.ThemeBlueColors
import com.quran.app.compose.theme.colors.ThemeDefaultColors
import com.quran.app.compose.theme.colors.ThemeMonoColors
import com.quran.app.compose.theme.colors.ThemePurpleColors
import com.quran.app.compose.theme.colors.ThemeRedColors
import com.quran.app.compose.theme.colors.ThemeVioletColors
import com.quran.app.compose.theme.colors.ThemeYellowColors
import com.quran.app.compose.utils.preferences.DataStoreManager
import com.quran.app.R
import com.quran.app.compose.utils.ThemeUtils.observeDarkTheme
...
object ThemeUtils {
    ...
    private fun ColorScheme.withLiquidGlass(isDarkTheme: Boolean): ColorScheme {
        return copy(
            ...
            surface = surface.copy(alpha = if (isDarkTheme) 0.40f else 0.60f),
            surfaceContainerLowest = surfaceContainerLowest.copy(alpha = if (isDarkTheme) 0.15f else 0.25f),
            surfaceContainerLow = surfaceContainerLow.copy(alpha = if (isDarkTheme) 0.25f else 0.35f),
            surfaceContainer = surfaceContainer.copy(alpha = if (isDarkTheme) 0.50f else 0.70f),
            surfaceContainerHigh = surfaceContainerHigh.copy(alpha = if (isDarkTheme) 0.65f else 0.80f),
            surfaceContainerHighest = surfaceContainerHighest.copy(alpha = if (isDarkTheme) 0.80f else 0.90f),
            outline = outline.copy(alpha = if (isDarkTheme) 0.45f else 0.60f),
            outlineVariant = outlineVariant.copy(alpha = if (isDarkTheme) 0.25f else 0.40f),
        )
    }
