package com.quran.app.utils.managers

import com.quran.app.utils.workers.TranslationDownloadWorker
...
object TranslationDownloadManager {
    ...
    private val downloadStates = MutableStateFlow<Map<String, WorkInfo>>(emptyMap())
    ...
    private fun updateState(slug: String, info: WorkInfo) {
        downloadStates.update { it + (slug to info) }
    }

    private fun removeState(slug: String) {
        downloadStates.update { it - slug }
    }

    fun isDownloading(slug: String): Boolean {
        val workInfo = downloadStates.value[slug] ?: return false
        return workInfo.state == WorkInfo.State.ENQUEUED || workInfo.state == WorkInfo.State.RUNNING
    }

    fun isAnyDownloading(): Boolean {
        return downloadStates.value.values.any {
            it.state == WorkInfo.State.ENQUEUED || it.state == WorkInfo.State.RUNNING
        }
    }

    @OptIn(ExperimentalCoroutinesApi::class)
    fun observeDownloadsAsFlow(): Flow<Pair<String, ResourceDownloadStatus>> =
        downloadStates.flatMapLatest { map ->
            flow {
