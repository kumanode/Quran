package com.quran.app.utils.reader.tafsir

import android.content.Context
import com.quran.app.api.JsonHelper
import com.quran.app.api.RetrofitInstance
import com.quran.app.api.models.tafsir.AvailableTafsirsModel
import com.quran.app.api.models.tafsir.TafsirInfoModel
    ...
    private suspend fun postTafsirsLoad(
        ...
        try {
            ...
            if (idList.none { it.key == "id_kemenag" }) {
                ...
                CoroutineScope(Dispatchers.IO).launch {
                    try {
                        val dbHelper = com.quran.app.db.tafsir.QuranTafsirDBHelper(ctx)
                        dbHelper.storeTafsirInfo(indonesianTafsir)
                        
                        val db = dbHelper.readableDatabase
                        val count = android.database.DatabaseUtils.queryNumEntries(
                            db, 
                            com.quran.app.db.tafsir.QuranTafsirContract.QuranTafsirEntry.TABLE_NAME, 
                            "${com.quran.app.db.tafsir.QuranTafsirContract.QuranTafsirEntry.COL_TAFSIR_KEY} = ?", 
                            arrayOf("id_kemenag")
                        )
                        ...
                        if (count < 6000) {
                            ...
                            val tafsirs = mutableListOf<com.quran.app.api.models.tafsir.TafsirModel>()
                            while (iter.hasNext()) {
                                val key = iter.next()
                                tafsirs.add(com.quran.app.api.models.tafsir.TafsirModel(
                                    key = "id_kemenag",
                                    verseKey = key,
                                    verses = listOf(key),
